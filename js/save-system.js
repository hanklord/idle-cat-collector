(function(){
  ...
  function fresh(){return{coins:50,owned:{orange_tabby:1},levels:{},lastTimestamp:Date.now()}}
  function load(){
    var base=fresh();
    try{
  ...
  Save.prototype.save=function(){this.data.coins=this.game.coins;this.data.lastTimestamp=Date.now();this.data.owned=this.game.owned;this.data.levels=this.game.levels;try{localStorage.setItem(KEY,JSON.stringify(this.data))}catch(e){}};
